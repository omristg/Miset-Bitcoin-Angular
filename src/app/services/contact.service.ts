...
export class ContactService {
	...
	private _filterBy$ = new BehaviorSubject<FilterBy>(null)
	...
	public loadContacts(): void {
		...
		this._saveToStorage(this._contactsDb)
		if (filterBy) {
	...
	private _filter(contacts: Contact[], filterBy: FilterBy) {
		const nameRegex = new RegExp(filterBy.name, 'i')
		const emailRegex = new RegExp(filterBy.email, 'i')
		const phoneRegex = new RegExp(filterBy.phone, 'i')
	...
	private _saveToStorage(entities: object[]) {
		localStorage.setItem(KEY, JSON.stringify(entities))
	}

	private _loadFromStorage() {
		const etitiesJson = localStorage.getItem(KEY)
		if (etitiesJson) return JSON.parse(etitiesJson)
	}
}